...
void tambahFile()
{
	...
	ofstream gas(berkas.c_str(), ios::out); //menambah isi file
	if(gas.is_open())
	{
		cin.ignore();
		cout << "Masukkan tulisan yang ingin diinput ke dalam file \n";
		getline(cin, teks);
		gas << teks << endl;
		gas.close();
		cout << "Tulisan berhasil diinput kedalam berkas \"" << berkas << "\"\n";
...
void bacaFile()
{
	...
	ifstream gas(berkas.c_str(), ios::out); //menambah isi file
	if(gas.is_open())
	{
